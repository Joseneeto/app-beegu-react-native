...
const EditarCarona = ({ route, navigation }) => {
  ...
  React.useEffect(() => {
    fetch(`${endpoint}/${id}`)
      ...
      .then(data => {
        setaHorario(data.horario);
        setaVagas(data.vagas);
        setaEnderecoInicial(data.enderecoInicial);
        setaEnderecoFinal(data.enderecoFinal);
      });
  }, []);
  ...
  const atualizarCaronaById = (id) => {
    fetch(`${endpoint}/${id}`, {
      ...
      Alert.alert('Carona atualizada com sucesso!')
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ImageBackground source={require('../assets/images/logo.png')} style={styles.rideDetails} imageStyle={styles.image}>
        <ScrollView style={styles.scroll} contentContainerStyle={{ paddingBottom: 210 }}>
          ...
          <TouchableHighlight style={styles.botao} underlayColor="#674461" onPress={ () => {atualizarCaronaById(id)} }>
            <Text style={styles.txtBotao}>Atualizar carona</Text>
          </TouchableHighlight>
